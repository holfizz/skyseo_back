...
export class PaymentsService implements OnModuleInit {
	...
	async handleYooKassaWebhook(body: any) {
		...
		if (event === 'payment.succeeded') {
			...
			// Начисляем баллы. Это обязательно: очередь задач отсекает владельцев,
			// у которых баланса не хватает на визит (computeAvailability), поэтому
			// без начисления оплата не возвращает сайт в выдачу — деньги ушли, а
			// продвижение стоит. Ровно так работало до перехода на подписку.
			...
			// Дату оплаты тоже двигаем — её показывает веб-кабинет, на выдачу задач
			// она не влияет.
